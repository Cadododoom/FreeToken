...
def iter_weights(
    ...
    *,
    ...
    tp_config = parse_config(hf_config) if tp.size > 1 else None
    for file in tqdm(
        ...
        with safetensors.safe_open(file, framework="pt", device=str(device)) as f:
            for raw_name in f.keys():
                ...
                if fused is None:
                    ...
                    yield name, _shard(name, tensor, tp_config, tp.rank, tp.size)
                else:
                    for fused_name, fused_tensor in fused:
                        yield fused_name, _shard(fused_name, fused_tensor, tp_config, tp.rank, tp.size)
