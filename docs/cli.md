...
| `--quant-backend` | auto | Kernel per quantized layer type, `layer[.kind]=name` entries: `linear=marlin,moe=b12x`, `moe.nvfp4=triton`, or `moe.awq=marlin`. A layer-level entry applies to every kind whose table lists the name |
...
| `--expert-load` | auto | `auto`/`serial`/`parallel` host-bank read path; auto avoids the parallel reader when its transient shard buffer would exceed available RAM |
| `--moe-shared-bank-dir` | — | Optional model-specific shared-tmpfs cache that packs routed experts once and maps the same host pages into independent endpoints; use a large `/dev/shm` path; raw checkpoint repacks only |
...
| `--moe-pageable-staging` | off | Explicitly keep all MoE layers on the eager GPU offload path when expert banks are pageable/file-backed; only fetched rows are synchronously copied into the GPU LRU cache. Disables CUDA graphs and MoE prefill overlap, so expect lower decode throughput than registered-host offload |
